...
{% raw %} {% url 'name' %}{% rawend %}
带参数的：参数可以是变量名
{% raw %} {% url 'name' 参数 %}{% rawend %} 

例如：
<a href="{% raw %} {% url 'add' 4 5 %}{% rawend %}">link</a>
渲染结果为：
...
<!DOCTYPE html>
<html>
<head><title>{% raw %} {% block title %}{% rawend %}默认标题{% raw %} {% endblock %}{% rawend %} - 自强学堂</title>
</head>
<body> 
{% raw %} {% include 'nav.html' %}{% rawend %} 
{% raw %} {% block content %\
}<div>这里是默认内容，所有继承自这个模板的，如果不覆盖就显示这里的默认内容。</div>
{% raw %} {% endblock %}{% rawend %} {% raw %} {% include 'bottom.html' %}{% rawend %} 
{% raw %} {% include 'tongji.html' %}{% rawend %}
</body>
</html>
```
...
```django
{% raw %} {% extends 'base.html' %}{% rawend %} 
{% raw %} {% block title %}{% rawend %}欢迎光临首页{% raw %} {% endblock %}{% rawend %} 
{% raw %} {% block content %}{% rawend %}
{% raw %} {% include 'ad.html' %}{% rawend %}
这里是首页，欢迎光临
{% raw %} {% endblock %}{% rawend %}
```
...
在模板中，可以使用`\{\{\}\}`引用变量，用`{% raw %} {% %}{% rawend %}`插入语句。
...
- for循环和list
  ...
  ```django
  <!-- home.html -->
  {% raw %} {% for i in TutorialList %}{% rawend %}
  \{\{ i \}\}
  {% raw %} {% endfor %}{% rawend %}
  ```
...
- 字典
  ...
  ```django
  <!-- home.html -->
  {% raw %} {% for key, value in info_dict.items %}{% rawend %}    
  \{\{ key \}\}: \{\{ value \}\}
  {% raw %} {% endfor %}{% rawend %}
  ```
...
- 逻辑与比较操作
  ...
  ```django
  {% raw %} {% if num <= 100 and num >= 0 %}{% rawend %}
  num在0到100之间
  {% raw %} {% else %}{% rawend %}
  数值不在范围之内！
  {% raw %} {% endif %}{% rawend %}
...
然后在HTML文件开头加上```{% raw %} {% load static %}{% rawend %}```就可以如下引用：

```html
<link href="{% raw %} {% static "css/index.css" %}{% rawend %}" rel="stylesheet">
<script src="{% raw %} {% static "js/scrollReveal.js" %}{% rawend %}"></script>
```
